...
trans[0][0]=n;
trans[0][1]=m; int k=1;
trans[0][2]=sm[0][2]; for(int i=0;i<n;i++)
{
...
int i,j,k,m,n; if((sm[0][0]!=sm1[0][0])&&(sm[0][1]!=sm1[0][1]))
{
...
add[0][0]=sm1[0][0];
add[0][1]=sm1[0][1]; i=1;j=1;k=0; m=sm[0][2];
n=sm1[0][2];
while(i<=m && j<=n)
{
...
add[k][0]=sm[i][0];
add[k][1]=sm[i][1]; add[k][2]=sm[i][2]+sm1[j][2];
i++; j++;
}
...
int arr[100][100]; int arr1[100][100]; int m, n;
int sm[50][3];
int sm1 [50][3]; int ch;
int add[100][3];
...
case 1:
printf("\nEnter the no. of rows "); scanf("%d", &m);
printf("\nEnter the no. of columns "); scanf("%d", &n);
accept(arr, m, n); break;
